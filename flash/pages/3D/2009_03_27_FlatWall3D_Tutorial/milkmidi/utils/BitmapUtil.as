...
/*
		public static function reflect1(pTarget:*, pAttach:Boolean = false, pRatioFade:Number = .5, pAlpha:Number = .5):BitmapData	{
			//var _targetBmp:BitmapData = BitmapUtil.getBitmapData(pTarget);
			if ( !(pTarget is BitmapData) && !(pTarget is DisplayObject)) throw new Error("error Type");			
			
			var _resultBmp		:BitmapData = new BitmapData(pTarget.width, pTarget.height, true, 0);			
			var _drawMatrix		:Matrix = new Matrix();
			//var matSkew:Matrix = new Matrix(1, 0, 0, -1 * _numScale, 0, p_target.height);			
			var _recDraw		:Rectangle = new Rectangle(0, 0, pTarget.width, pTarget.height);
			
			
			_drawMatrix.scale( 1, -1 );
			//_drawMatrix.scale( pTarget.scaleX, -pTarget.scaleY );
			
			_drawMatrix.translate(0, pTarget.height);			
			//_resultBmp.draw( pTarget, _drawMatrix, pTarget.transform.colorTransform, BlendMode.LAYER );
			_resultBmp.draw( pTarget, _drawMatrix, null, null, _recDraw, true);						
			
			var _gradientShape	:Shape = new Shape();			
			var _fillType		:String = GradientType.LINEAR;
			var _colorsArray	:Array = [0xffffff, 0x0];						
			var _alphasArray	:Array = [0, pAlpha];						
			//* ration available  0 ~ 0xff 
            var _ratiosArray	:Array = [0, 0xff * pRatioFade];	
			
			var _matrix			:Matrix = new Matrix();
			_matrix.createGradientBox(_resultBmp.width, _resultBmp.height, Math.PI / 2, 0, 0);							
			//_matrix.createGradientBox(_resultBmp.width, _resultBmp.height, Math.PI / 2, 0, 0);				
			_gradientShape.graphics.beginGradientFill(_fillType, _colorsArray , _alphasArray, _ratiosArray, _matrix);			
			_gradientShape.graphics.drawRect(0, 0, _resultBmp.width, _resultBmp.height);			
			//_resultBmp.draw(_gradientShape, null, null, BlendMode.ALPHA);				
			_resultBmp.draw(_gradientShape, _drawMatrix, null, BlendMode.ALPHA);				
			
			
			//var _alphaBmp:BitmapData = new BitmapData(pTarget.width, pTarget.height, true, 0);
			//_alphaBmp.draw(_gradientShape);
			//_resultBmp =  BitmapUtil.mergeBitmapAlpha(_resultBmp , _alphaBmp);
