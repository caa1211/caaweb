...
if (jQuery) (function(jQuery){
	...
	jQuery.extend(jQuery.fn, {
		gsFileManager: function(o) {
			...
			jQuery('#gs_image_x').bind('blur', function () {
				manageImgSizes('blur', 'x');		
			});
			
            jQuery('#gs_image_y').bind('blur', function () {
            	manageImgSizes('blur', 'y');	
			});
            
            jQuery('#gs_image_x').bind('focus', function () {
            	manageImgSizes('focus', 'x');	
			});
			
            jQuery('#gs_image_y').bind('focus', function () {
            	manageImgSizes('focus', 'y');				
			});
			...
			function manageImgSizes(operation, direction){
				var Xelement = jQuery('#gs_image_x');
				var Yelement = jQuery('#gs_image_y');
				if (operation == 'focus'){
					if (direction == 'x') {
						if (!isNaN(Xelement.val())) {
						    Xelement.attr('rel', Xelement.val());
						}
					}
					if (direction == 'y') {
						if (!isNaN(Yelement.val())) {
						    Yelement.attr('rel', Yelement.val());
						}
					}
				}
				if (operation == 'blur'){
			...
			function setHandlers(t) {
				...
				jQuery(t).find('LI > A').bind('click', function () {
					showTree (jQuery(this).parent(), escape(jQuery(this).attr('rel').match( /.*\// )));
		...
		doGSAction: function(o) {
			...
			function showImageViewer(o, curDir, gsitem){
				...
				jQuery('#gsimageviewer_content').html('<img src="'+ imageSrc + '" id="gs_imageviewer_image"/>');
				jQuery('#gs_imageviewer_image').load( function(){
					jQuery('#gs_image_x').val(jQuery(this).width());
					jQuery('#gs_image_x').attr('rel', jQuery(this).width());
					jQuery('#gs_image_y').val(jQuery(this).height());
					jQuery('#gs_image_y').attr('rel', jQuery(this).height());
				});
			}
