...
class GSFileManager {
	...
	public function process($args){
		...
		if (!isset($args['dir'])) {
		    ...
		    if (get_magic_quotes_gpc() || get_magic_quotes_runtime()) {
				$args['dir'] = stripslashes($args['dir']);
			}
			$args['dir'] = urldecode($args['dir']);
		}
	    if (isset($args['filename'])) {
			$args['filename'] = basename(html_entity_decode(urldecode($args['filename']), ENT_QUOTES, 'UTF-8'));
			if (get_magic_quotes_gpc() || get_magic_quotes_runtime()) {
				$args['filename'] = stripslashes($args['filename']);
			}
			$this->checkFileName($args['filename']);
		}
	    if (isset($args['newfilename'])) {
			$args['newfilename'] = basename(urldecode($args['newfilename']));
	        if (get_magic_quotes_gpc() || get_magic_quotes_runtime()) {
				$args['newfilename'] = stripslashes($args['newfilename']);
			}
	...
	public function unZipItem($args){
		$root = $this->getOptionValue(self::$root_param);
		if (isset($args['filename'])) {
			$filename = html_entity_decode(urldecode($args['filename']), ENT_QUOTES, 'UTF-8');
			$dir = urldecode($args['dir']);
    ...
    public function zipItem($args) {
		...
		if (isset($args['filename'])) {
			$filename = html_entity_decode(urldecode($args['filename']), ENT_QUOTES, 'UTF-8');
			$dir = html_entity_decode(urldecode($args['dir']), ENT_QUOTES, 'UTF-8');
		...
		if (isset($args['filename'])) {
			...
			if ($this->fileStorage->file_exists($root.$dir.$args['filename'])) {
				...
				if (!$this->fileStorage->file_exists($root.$dir.$newFilename)) {
					...
					if ($this->fileStorage->writefile($root.$dir.$newFilename, $content)){
						return '{result: \'1\'}';
					}
					return '{result: \'0\'}';
				}else {
	...
	public function writefile($args) {
		...
		if (isset($args['filename'])) {
			...
			$dir = urldecode($args['dir']);
			if ($this->fileStorage->file_exists($root.$dir.$filename)) {
				if($this->fileStorage->writefile($root.$dir.$filename, $this->fixString($content))){
					return '{result: \'1\'}';
				}
				return '{result: \'0\'}';
			} else {
	...
	public function moveItems($args) {
		...
		if (isset($args['files'])) {
			...
			foreach ($files as $filename) {
				$filename = html_entity_decode(urldecode($filename), ENT_QUOTES, 'UTF-8');
				if (get_magic_quotes_gpc() || get_magic_quotes_runtime()) {
	...
	public function renameItem($args) {
		...
		if (isset($args['filename'])) {
			...
			if ($this->fileStorage->file_exists($root.$dir.$filename)) {
				if (!$this->fileStorage->file_exists($root.$dir.basename($newFileName))) {
					if ($this->fileStorage->renameItem($root.$dir.$filename, $root.$dir.basename($newFileName))) {
						return '{result: \'1\'}';
					}
					return '{result: \'0\'}';
				} else {
	...
	public function deleteItem($args) {
		...
		if (isset($args['files'])) {
			...
			$responce = '{result: \'1\'}';
			foreach ($files as $filename) {
				$filename = basename(html_entity_decode(urldecode($filename), ENT_QUOTES, 'UTF-8'));
			    if (get_magic_quotes_gpc() || get_magic_quotes_runtime()) {
				   $filename = stripslashes($filename);
			    }
				$this->checkFileName($filename);
				if ($this->fileStorage->file_exists($root.$dir.$filename)) {
					if ($this->fileStorage->is_dir($root.$dir.$filename)) {
						if (!$this->fileStorage->deleteDirectory($root.$dir.$filename)) {
							throw new Exception('ServerException: can NOT delete '.$filename, 9);
						}
					} else {
	...
	public function listDir($args) {
		...
		if( $this->fileStorage->file_exists($root . $dir) ) {
			...
			throw new Exception('ILlegalArgumentException: dir to list does NOT exists', 3);
		}
	}
...
$options[GSFileManager::$root_param] = 'C:/amp/ap2_2/htdocs/jtreefs';
$manager = new GSFileManager(new GSFileSystemFileStorage(), $options);
try {
	$result = $manager->process($_REQUEST);
} catch (Exception $e) {
	$result = '{result: \'0\', gserror: \''.$e->getMessage().'\', code: \''.$e->getCode().'\'}';
}
echo $result;
?>